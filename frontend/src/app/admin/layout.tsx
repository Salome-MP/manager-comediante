...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (ready && (!user || (user.role !== 'SUPER_ADMIN' && user.role !== 'STAFF'))) {
      router.push('/login');
    }
  }, [ready, user, router]);
    ...
    <div className="flex h-screen overflow-hidden bg-surface-deep">
      ...
      >
          ...
          {!collapsed && (
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col leading-none">
                ...
                <span className="text-[10px] text-text-dim">Panel de control</span>
              </div>
            </div>
          )}
          {collapsed && (
            <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br from-navy-600 to-navy-800 shadow-lg shadow-navy-900/50 mx-auto">
              <Shield className="h-3.5 w-3.5 text-white" />
            </div>
          )}
          {!collapsed && (
        ...
        <nav className="flex-1 space-y-0.5 overflow-y-auto p-2 py-3">
          {collapsed && (
            <button
              className="flex w-full items-center justify-center rounded-md py-2 text-text-dim hover:bg-overlay-light hover:text-text-primary transition-colors mb-1"
